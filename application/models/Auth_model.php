...
class Auth_model extends CI_Model{
	...
	{
		$query = $this->db->get_where('rac_users', array('email' => $data['email']));
		if ($query->num_rows() == 0){
			return false;
		}
		else{
	...
	public function email_verification($code)
	{
		...
		if($match > 0){
			$this->db->where('token', $code);
			$this->db->update('rac_users', array('is_verify' => 1, 'token'=> ''));
			return $result;
		}
		else{
			return false;
    ...
    {
    	$result = $this->db->get_where('rac_users', array('email' => $email));
    ...
    public function check_password_reset_code($code)
    {
    	...
    	$result = $this->db->get_where('rac_users',  array('password_reset_code' => $code ));
    	if($result->num_rows() > 0){
    		return true;
    	}
    	else{
