...
class Cv_Model extends CI_Model{
	...
	public function do_shortlist($emp_id, $user_id)
	{
	    ...
	    $query = $this->db->get('rac_cv_shortlisted');
	    if ($query->num_rows() > 0){
	        return true;
	    }
	    else{
	        $this->db->insert('rac_cv_shortlisted', array('employer_id' => $emp_id, 'user_id' => $user_id));
			return true;
	    }
	...
	{
		$this->db->select('rac_cv_shortlisted.id, 
			rac_cv_shortlisted.user_id,
			rac_cv_shortlisted.employer_id,
			rac_users.id as seeker_id,
			rac_users.firstname, 
			rac_users.lastname,
			rac_users.email,
			rac_users.city,
			...
			rac_users.resume');
		$this->db->from('rac_cv_shortlisted');
		$this->db->join('rac_users','rac_users.id = rac_cv_shortlisted.user_id','left');
		$this->db->where('rac_cv_shortlisted.employer_id', $this->session->userdata('employer_id'));
		$this->db->order_by("rac_cv_shortlisted.created_date", "DESC");
		$query = $this->db->get();
		$module = array();
		if ($query->num_rows() > 0) 
		{
