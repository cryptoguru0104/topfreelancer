...
class Mailer 
{
	...
	//=============================================================
    // Email Templates
    function mail_template($from = '', $to = '',$slug = '',$mail_data = '', $file='')
    {

		$template =  $this->CI->db->get_where('rac_email_templates',array('slug' => 'email-verification'))->row_array();
        ...
        sendEmail($from, $to,$subject,$template, $file = '');
    ...
    function mail_template_variables($content,$slug,$data = '')
    {
        ...
        switch ($slug) {
            ...
            case 'freelancer-applied':
                $content = str_replace('{JOB_TITLE}',$data['job_title'],$content);
                $content = str_replace('{JOB_EMP}',$data['job_emp'],$content);
                $content = str_replace('{JOB_DATE}',$data['job_date'],$content);
                $content = str_replace('{APPLY_DATE}',$data['apply_date'],$content);
                $content = str_replace('{FL_NAME}',$data['fl_name'],$content);
                $content = str_replace('{FL_EMAIL}',$data['fl_email'],$content);
                $content = str_replace('{FL_PHONE}',$data['fl_phone'],$content);
                $content = str_replace('{COVER_LETTER}',$data['cover_letter'],$content);
                return $content;
            break;
    ...
    function send_verification_email($id,$type = '')
    {
        if($type == 'user' || $type == '')
        {
            $user = $this->CI->db->get_where('rac_users',array('id' => $id))->row_array();
        }
        ...
        // Get Email Template
        $temp =  $this->CI->db->get_where('rac_email_templates',array('slug' => 'email-verification'))->row_array();

		$from = 'noreplay@topfreelancer';
        $to = $user['email'];
        ...
        sendEmail($from, $to,$temp['subject'],$template);
	...
	function pwd_reset_link($username, $reset_link)
	{
		$tpl = '<h3>Hi ' .strtoupper($username).'</h3>
            ...
            <p>© 2019 Topfreelancer - All rights reserved</p>
    ';
		return $tpl;		
	}
