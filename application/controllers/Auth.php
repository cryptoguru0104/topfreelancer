...
class Auth extends Main_Controller {
		...
		{ 
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($result) {
					...
					{
						$login_data['event_id'] = $this->session->userdata('event_id');
					}
	...
	public function registration()
	{
		...
		if ($this->input->post('submit')) 
		{
			...
			$this->form_validation->set_rules('email','email','trim|required|min_length[5]|valid_email|is_unique[rac_users.email]');
			$this->form_validation->set_rules('password','password','trim|required|min_length[3]');
	...
	public function forgot_password()
	{
		...
		if($this->input->post('submit')){
			...
			if($response){
				...
				$message =  $body ;
				$email = sendEmail($from, $to, $subject, $message, $file = '' , $cc = '');
